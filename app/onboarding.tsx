/**
 * PANTALLA: OnboardingScreen (Expo Router)
 * Propósito: Flujo de 4 preguntas para personalizar la experiencia (según README)
 * Navegación: Inicial → (tabs)/index (al completar)
 */
...
import { ONBOARDING_OPTIONS, UserPreferences } from '../src/models/UserPreferences';
...
export default function OnboardingScreen() {
  ...
  const handleOptionSelect = (optionKey: string) => {
    const updatedPrefs = new UserPreferences({
      ...preferences,
  ...
  const completeOnboarding = async (finalPreferences: any) => {
    try {
      ...
      Alert.alert('Error', 'Hubo un problema guardando tus preferencias');
    }
  };
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.optionsContainer}>
          {currentQuestion.options.map((option) => {
            const isSelected = (preferences as any)[currentQuestion.id] === option.key;
